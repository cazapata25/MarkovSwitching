...
# Convierte en series de tiempo de datos anuales
...
windows()
plot(bt[2:76],st[3:77],ylab ="Primary Balance/GDP", xlab="Lagged public debt/GDP",pch=20,ylim = c(-0.3,0.1),col="darkblue")
abline(lm(st[3:77]~bt[2:76]),col="darkblue")
...
ggplot(df1) + 
    ...
    ggtitle("Fig 2. Federal debt and primary federal surplus in the US (1940-2016)")
...
st <- ts(st[3:77])
lagbt <- ts(bt[2:76])
gdp.gapt <- ts(gdp.gap[3:77])
g.cyclet <- ts(g.cycle[3:77])
...
filtprob.res = ts(filtprob[,2],start=c(1942),end=c(2016),freq=1)
smoprob=(model1.MS@Fit@smoProb) # Smoothed probability
smoprob.res = ts(smoprob[,2],start=c(1942),end=c(2016),freq=1)
...
df5 <- data.frame(rtadj[2:77],years)
df6 <- data.frame(bp.rits,years)
